...
## --- msbwtAnalyze/stack_query.py --- ##
##      Date: 17 March 2017
##      Purpose: given probe files, query an msbwt using a stack implementation 
...
def generate_counts(bwtfile, probe_files, out_dir):
    ...
    for probe_file in p_files:
        ...
        if os.path.isfile('{}/{}'.format(out_dir, os.path.basename(probe_file))):
            ...
            # Hack to exit if running cluster machine is too slow
            if float(counter)/(finish-start) < 11.0:
                print 'Exiting... Machine too slow.'
                break

def run(bwtfile, probe_files, out_dir):
    if os.path.isfile('{}/running.txt'.format(out_dir)):
        sys.exit()
    else: # BWT is not currently running
        target = open('{}/running.txt'.format(out_dir), 'w')
        target.write(socket.gethostname()+'\n')
        target.close()
        generate_counts(bwtfile, probe_files, out_dir)
        os.remove('{}/running.txt'.format(out_dir))
...
if __name__ == '__main__':
    ...
    run(args.bwt_file, args.probe_files, args.out_dir)
